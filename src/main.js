...
function updateLanguageButtons() {
  document.querySelectorAll('[data-lang-btn]').forEach(button => {
    ...
    if (lang === currentLang) {
      button.classList.remove('text-slate-400', 'hover:text-slate-600', 'dark:hover:text-slate-200');
      button.classList.add('text-primary');
    } else {
      button.classList.remove('text-primary');
      button.classList.add('text-slate-400', 'hover:text-slate-600', 'dark:hover:text-slate-200');
    }
  });
}
