...
import { Eye, Trash2, Plus, ChevronLeft, ChevronRight, X, ImageIcon, Loader2, AlertCircle } from 'lucide-react';
...
export default function Gallery({ dict }: { dict: any }) {
    ...
    const nextImage = () => currentIndex !== null && setCurrentIndex((currentIndex + 1) % galleries.length);
    const prevImage = () => currentIndex !== null && setCurrentIndex((currentIndex - 1 + galleries.length) % galleries.length);
            ...
            <div className="min-h-screen p-10">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
                        <div key={i} className="aspect-[4/3] rounded-3xl bg-slate-200 dark:bg-slate-800 animate-pulse"></div>
                    ))}
                </div>
            </div>
        );
    ...
    if (error) {
        return (
            <div className="min-h-screen flex flex-col items-center justify-center p-10 text-center">
                ...
                <p className="text-slate-500 mb-6">Impossible de charger les images pour le moment.</p>
    ...
    return (
        <div className="min-h-screen p-6 lg:p-10 font-sans text-text-main relative">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
                <div>
                    <h1 className="text-3xl font-black tracking-tight">{t.title || "Gallery"}</h1>
                    <p className="text-slate-500 mt-1">{t.subtitle || "Gérez vos ressources visuelles."}</p>
                </div>
                <button 
            ...
            {galleries.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-20 border-2 border-dashed border-card-border rounded-[3rem] bg-slate-500/5">
                    ...
                    <p className="text-slate-500 font-medium">Aucune image dans la galerie pour le moment.</p>
                </div>
            ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    <div className="w-full max-w-5xl px-4 flex flex-col items-center">
                        <div className="relative group w-full aspect-video md:aspect-[16/9] rounded-[2rem] overflow-hidden shadow-2xl border border-white/10">
                            <img 
                                ...
                                alt={galleries[currentIndex].title || 'Gallery Image'}
                                className="w-full h-full object-contain"
                            />
                        </div>
                        
                        <div className="mt-8 text-center text-white space-y-2">
                            <h2 className="text-2xl font-black">{galleries[currentIndex].title}</h2>
                            <p className="text-primary font-bold uppercase tracking-widest text-sm">{galleries[currentIndex].category}</p>
                            <p className="text-slate-400 max-w-2xl mx-auto italic">{galleries[currentIndex].description}</p>
