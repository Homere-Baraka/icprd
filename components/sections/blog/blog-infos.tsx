...
export default function BlogInfos() {
    ...
    const allPublishedBlogs = blogs?.data?.filter(
        (blog: any) => blog.publishedAt !== null,
    ) || [];

    const startIndex = currentIndex * ITEMS_PER_PAGE;
    const visibleBlogs = allPublishedBlogs.slice(startIndex, startIndex + ITEMS_PER_PAGE);
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    {isLoading ? (
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                            {[1, 2, 3].map((n) => (
                                <div key={n} className="h-[400px] bg-card-border/5 animate-pulse rounded-2xl" />
                            ))}
                        </div>
                    ) : (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                            ...
                            {visibleBlogs.map((blog: any) => (
                                <article key={blog.id} className="group blog-card flex flex-col bg-white dark:bg-slate-900 rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
                                        ...
                                        <div className="p-6 flex-1 flex flex-col">
                                            ...
                                            <div className="mt-auto pt-6 border-t border-white/5 flex items-center justify-between">
                                                <div className="flex items-center gap-3">
                                                    <div className="size-8 rounded-full bg-primary/20 flex items-center justify-center text-primary"><User size={16} /></div>
                                                    <div className="flex flex-col">
                                                        <span className="text-xs font-bold text-white">{blog.author?.name || 'Équipe ICPRD'}</span>
                                                        <span className="text-[10px] text-slate-500">
                                                            {blog.createdAt && formatDistanceToNow(new Date(blog.createdAt), { addSuffix: true, locale: fr })}
                                                        </span>
                                                    </div>
                                                </div>
